$ScriptVersion = '$Header: /CVS/openmake64/tomcat/webapps/openmake.ear/openmake.war/scripts/Yacc.sc,v 1.2 2005/06/06 16:18:05 jim Exp $';
...
#-- Note: this script won't work for Bison (use 'GNU Bison.sc' )
#   and doesn't handle header files (use 'Yacc Header Generate.sc')

#-- Special paramters this script handles:
#
# AdditionalFlags:  yaccreplace=<symbol>
#
#   will replace yy with lc(<symbol>) and YY with uc(<symbol>)
#
...
chmod 0777, 'ytab.c', 'y.tab.c';
unlink 'ytab.c', 'y.tab.c';
...
#-- Try to find the output file, yacc is not standard
if ( $RC == 0 ) {
 if ( -f 'y.tab.c' ) {
  push @CompilerOut, "\n", "move y.tab.c, $TargetFile", "\n";
 
  unless (move 'y.tab.c', $TargetFile ) {
   push @CompilerOut, "$!";
   $RC = 1;
  }
 } elsif ( -f 'ytab.c' ) {
  push @CompilerOut, "\n", "move ytab.c, $TargetFile", "\n";
 
  unless (move 'ytab.c', $TargetFile ) {
   push @CompilerOut, "$!";
   $RC = 1;
  }
