...
#   Last updated:
#    ADG - 02.08.06 - removed all xmi scrubbing logic due to its disruption of setting xmi dir value correctly in AntFilesetOrg
#    QAB - 09.30.05 - remove $xmi_intdir directory after successful compile
#    JAG - 02.11.05 - fixs
#    JAG - 12.08.04 - eliminate copylocal step
#    JAG - 07.13.04 - make 'build.xml' more meaningful
my $ScriptVersion = '$Header: /CVS/openmake64/tomcat/webapps/openmake.ear/openmake.war/scripts/Ant\040Ear\040Task.sc,v 1.19 2010/08/30 19:53:04 steve Exp $';
...
$StepDescription = "Ear File Creation for " . $Target->get;
...
#########################################
#-- Verify that all dependencies could be found
#
@DepSearchFound = ();
@DepSearchMissing = ();
@DepList = unique($TargetDeps->getList());
...
#   For example, in Ant Ear, we look for Manifest.mf, application.xml
...
foreach my $option_group ( @option_groups )
{
 ...
 my @TarDeps = $::AllDeps->getList();
 my @RelDeps = $::RelDeps->getList();
 my $tdep = "", $rdep = "";

  if (!defined %main::Target2RelTargetMap)
  {
   my $i = 0;
 ...
 #         in AntFilesetOrg
 foreach my $subdep ( @subtask_deps ) 
 {
 
  my $f = $main::RelTarget2TargetMap{$subdep};
  if ($f ne "")
  {
   my $f2 = $subdep;
   ...
   $TargetRelDeps->unshift($f);   
   $TargetDeps->unshift($f2);
  }
  else
  {
   $TargetRelDeps->push($subdep); 
  }
 }
