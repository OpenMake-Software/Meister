...
my $ScriptVersion = '$Header: /CVS/openmake64/tomcat/webapps/openmake.ear/openmake.war/scripts/Ant\040Jar\040Task\040Signed.sc,v 1.1 2009/12/07 21:00:12 layne Exp $';
...
########## Start Signed Jar logic ##########

# the following flags need to be added only to the <signjar> node if they are present.
@signJarOpts = ('alias','storepass','keystore','storetype','keypass','sigfile','signedjar','internalsf','sectionsonly','lazy','tsaurl','tsacert','preservelastmodified','maxmemory');

 my @tempopts;
foreach $tempopt(@options)
{
 $optKey = $tempopt;
 $optKey =~ s{^([^=\"]+)=[^=]+}{$1};
 if(grep (/$optKey/, @signJarOpts))
 {
 	if($optKey !~ m{signedjar}i) # we do not want to rename the target, therefore we will disallow "signedjar"
 	{
 		push @signJarTaskOpts, $tempopt; # create separate array for signed jar options, these will not be passed along to jar task
 	}
 }
 else
 {
	push @tempopts, $tempopt;
 }
}
@options = @tempopts;
	...
	if(@signJarTaskOpts)
	{
		foreach $sjopt(@signJarTaskOpts)
		{
			...
			if ($sjopt =~ m{([^=]+)=([^=]+)})
			{
				$sjkey = $1;
				$sjval = $2;
				$sjval = "\"" . $sjval if($sjval !~ m{^\"});
				$sjval = $sjval . "\"" if($sjval !~ m{\"$});
				$sjopt = $sjkey . "=" . $sjval;
			}
		}

		$signJarTaskOpts = join " ",@signJarTaskOpts;
		$signJarXML = "signjar " . $signJarTaskOpts . "\n" . "fileset dir=\".\" includes=\"$E{$Target->get}\"/\n/signjar"
	}
	else
	{
		print "WARNING: You have not defined any options for the Sign Jar task.\n\n";
	}
}

########## End Signed Jar logic ##########
